...
mod tests {
    ...
    fn test_parse_size_comprehensive() {
        ...
        assert_eq!(parse_size("2.25GB").unwrap(), (2.25 * 1024.0 * 1024.0 * 1024.0) as usize);
    ...
    fn test_parse_size_error_cases() {
        let invalid_inputs = vec![
            ...
            "-1MB",
            "-5",
            "1 MB", // Space in middle
            "MB",
            "KB",


            "1e10MB", // Scientific notation
            "∞MB", // Unicode
    ...
    fn test_cli_error_cases() {
        let error_cases = vec![
            ...
            vec!["disk-speed-test", "benchmark", "/tmp", "--duration", "invalid"],
            vec!["disk-speed-test", "benchmark", "/tmp", "--output-format", "invalid"],
            vec!["disk-speed-test", "benchmark", "/tmp", "--sequential-block-size"], // Missing value
        ];
    ...
    fn test_parse_size_boundary_values() {
        ...
        assert_eq!(parse_size("0.1MB").unwrap(), (0.1 * 1024.0 * 1024.0) as usize);
    }
    ...
    fn test_cli_with_extreme_values() {
        ...
        let args = vec!["disk-speed-test", "benchmark", "/tmp", "--duration", "86400"]; // 24 hours
        let result = Cli::try_parse_from(args);
        assert!(result.is_ok());
    ...
    fn test_output_format_case_sensitivity() {
        ...
        for format_str in format_cases {
            let args = vec!["disk-speed-test", "benchmark", "/tmp", "--output-format", format_str];
            let result = Cli::try_parse_from(args);
        ...
        for format_str in invalid_formats {
            let args = vec!["disk-speed-test", "benchmark", "/tmp", "--output-format", format_str];
            let result = Cli::try_parse_from(args);
            assert!(result.is_err(), "Should have failed for format: {}", format_str);
        }
    }
