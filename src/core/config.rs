//! Configuration structures for benchmark tests
...
pub struct BenchmarkConfig {
    ...
    /// Block size for sequential tests (default: 4MB)
    pub sequential_block_size: usize,
    
    /// Block size for random tests (default: 4KB)
    pub random_block_size: usize,
    
    /// Duration for each test in seconds (default: 10)
    pub test_duration_seconds: u64,
    
    /// Whether to disable OS caching (default: true)
    pub disable_os_cache: bool,
    
    /// Size of test file in MB (default: 1024 = 1GB)
...
impl BenchmarkConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.sequential_block_size == 0 {
            return Err("Sequential block size must be greater than 0".to_string());
        }
        
        if self.random_block_size == 0 {
            return Err("Random block size must be greater than 0".to_string());
        }
        ...
        if !self.target_path.exists() {
            return Err(format!("Target path does not exist: {}", self.target_path.display()));
        }
...
mod tests {
    ...
    fn test_config_validation() {
        ...
        config.target_path = env::current_dir().unwrap();
