...
mod tests {
    ...
    fn test_calculate_speed_mbps() {
        ...
        let speed = StatisticsCollector::calculate_speed_mbps(1024 * 1024, Duration::from_secs(1));
        assert!((speed - 1.0).abs() < 0.001);

        // 2MB in 1 second = 2 MB/s
        let speed =
            StatisticsCollector::calculate_speed_mbps(2 * 1024 * 1024, Duration::from_secs(1));
        assert!((speed - 2.0).abs() < 0.001);

        // Test with fractional seconds
        let speed =
            StatisticsCollector::calculate_speed_mbps(1024 * 1024, Duration::from_millis(500));
        assert!((speed - 2.0).abs() < 0.001);
