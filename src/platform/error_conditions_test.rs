...
mod tests {
    ...
    fn test_device_enumeration_errors() {
        ...
        for error in error_scenarios {
            ...
            match (&error, &actual_error) {
                (PlatformError::DeviceEnumerationFailed(expected), PlatformError::DeviceEnumerationFailed(actual)) => {
                    assert_eq!(expected, actual);
                }
                (PlatformError::InsufficientPermissions(expected), PlatformError::InsufficientPermissions(actual)) => {
                    assert_eq!(expected, actual);
                }
                (PlatformError::UnsupportedPlatform(expected), PlatformError::UnsupportedPlatform(actual)) => {
                    assert_eq!(expected, actual);
                }
    ...
    fn test_direct_io_file_creation_errors() {
        ...
        for error in error_scenarios {
            mock.set_file_operation_result(
                test_path.clone(),
                MockFileResult::Error(error.clone())
            );
            ...
            match (&error, &actual_error) {
                ...
                _ => panic!("Error type mismatch: expected {:?}, got {:?}", error, actual_error),
            }
        }
    }
    ...
    fn test_direct_io_file_opening_errors() {
        ...
        for error in error_scenarios {
            ...
            assert!(read_result.is_err(), "Read opening should fail with simulated error");
            
            let write_result = mock.open_direct_io_file_instance(&test_path, true);
            assert!(write_result.is_err(), "Write opening should fail with simulated error");
        }
    }
    ...
    fn test_error_recovery_scenarios() {
        ...
        mock.simulate_error(PlatformError::DeviceEnumerationFailed("Temporary failure".to_string()));
    ...
    fn test_concurrent_error_handling() {
        ...
        mock.disable_error_simulation();
        let result = mock.list_storage_devices_instance();
        assert!(result.is_ok(), "Should recover after disabling error simulation");
    }
    ...
    fn test_large_file_size_errors() {
        ...
        mock.set_file_operation_result(
            test_path.clone(),
            MockFileResult::Error(PlatformError::IoError(
                std::io::Error::new(std::io::ErrorKind::OutOfMemory, "File too large")
            ))
        );
    ...
    fn test_unicode_path_errors() {
        ...
        mock.set_file_operation_result(
            unicode_path.clone(),
            MockFileResult::Error(PlatformError::IoError(
                std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid unicode in path")
            ))
        );
    ...
    fn test_network_filesystem_errors() {
        ...
        mock.simulate_error(PlatformError::IoError(
