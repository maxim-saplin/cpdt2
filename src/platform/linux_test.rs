...
mod tests {
    ...
    fn test_linux_device_enumeration() {
        ...
        assert!(!devices.is_empty(), "Linux should have at least one storage device");
        ...
        for device in &devices {
            ...
            assert!(device.available_space <= device.total_space, 
                    "Available space should not exceed total space for device {}", device.name);
            
            // Check device type is reasonable
            match device.device_type {
                DeviceType::HardDisk | DeviceType::SolidState | DeviceType::Removable | 
                DeviceType::Network | DeviceType::Unknown => {
    ...
    fn test_linux_app_data_directory() {
        ...
        let has_xdg_or_local = path_str.contains(".local/share") || 
                              std::env::var("XDG_DATA_HOME").is_ok();
        assert!(has_xdg_or_local, "Should follow XDG Base Directory specification");
    ...
    fn test_linux_proc_mounts_parsing() {
        ...
        for device in &devices {
            ...
            assert!(!mount_str.starts_with("/proc"), "Should not include /proc mounts");
            assert!(!mount_str.starts_with("/sys"), "Should not include /sys mounts");
            assert!(!mount_str.starts_with("/dev/pts"), "Should not include /dev/pts mounts");
        }
    }
    ...
    fn test_linux_device_type_detection() {
        ...
        let has_typed_device = devices.iter().any(|d| {
            matches!(d.device_type, DeviceType::HardDisk | DeviceType::SolidState | DeviceType::Unknown)
        });
    ...
    fn test_linux_xdg_data_home_support() {
        ...
        let app_data_dir = LinuxPlatform::get_app_data_dir().unwrap();
        assert!(app_data_dir.starts_with("/tmp/custom_xdg"), 
    ...
    fn test_linux_filesystem_type_filtering() {
        ...
        assert!(!LinuxPlatform::is_real_filesystem("/proc/something", "ext4"));
