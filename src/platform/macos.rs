            ...
            match read_dir(volumes_path) {
                ...
                Err(e) => {
                    return Err(PlatformError::DeviceEnumerationFailed(
                        format!("Failed to read /Volumes: {}", e)
                    ));
                }
            }
        }
...
mod tests {
    ...
    fn test_get_app_data_dir() {
        ...
        assert!(path.to_string_lossy().contains("Library/Application Support/disk-speed-test"));
    ...
    fn test_list_storage_devices() {
        ...
        assert!(has_root, "Root filesystem should be included in device list");
    ...
    fn test_determine_device_type() {
        ...
        // Test network paths
        assert_eq!(determine_device_type(Path::new("/Volumes/Network"), &statvfs), DeviceType::Network);
        
        // Test USB/External paths
        assert_eq!(determine_device_type(Path::new("/Volumes/USB Drive"), &statvfs), DeviceType::Removable);
        assert_eq!(determine_device_type(Path::new("/Volumes/External"), &statvfs), DeviceType::Removable);
        
        // Test RAM disk
        assert_eq!(determine_device_type(Path::new("/Volumes/RAM Disk"), &statvfs), DeviceType::RamDisk);
        
        // Test unknown
        assert_eq!(determine_device_type(Path::new("/Volumes/SomeVolume"), &statvfs), DeviceType::Unknown);
    }
