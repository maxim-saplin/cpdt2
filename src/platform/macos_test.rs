...
mod tests {
    ...
    fn test_macos_device_enumeration() {
        ...
        assert!(
            !devices.is_empty(),
            "macOS should have at least one storage device"
        );
        ...
        for device in &devices {
            ...
            if device.mount_point == Path::new("/") {
                assert!(
                    ...
                    "Root filesystem should have total space > 0"
                );
                assert!(
    ...
    fn test_macos_volumes_enumeration() {
        ...
        let volumes_devices: Vec<_> = devices
            .iter()
        ...
        for device in volumes_devices {
            assert!(
                device.mount_point.starts_with("/Volumes"),
    ...
    fn test_macos_root_filesystem_properties() {
        ...
        assert_eq!(
            root_device.name, "Macintosh HD",
            "Root filesystem should be named Macintosh HD"
        );
    ...
    fn test_macos_device_type_detection() {
        ...
        let has_ssd = devices
            .iter()
            .any(|d| d.device_type == DeviceType::SolidState);
    ...
    fn test_macos_device_type_determination() {
        ...
        assert_eq!(
            determine_device_type(Path::new("/Volumes/Network Share"), &statvfs),
            DeviceType::Network
        );
        assert_eq!(
            determine_device_type(Path::new("/Volumes/net-drive"), &statvfs),
            DeviceType::Network
        );
        ...
        assert_eq!(
            determine_device_type(Path::new("/Volumes/External Disk"), &statvfs),
            DeviceType::Removable
        );
        ...
        // Test unknown
        assert_eq!(
            determine_device_type(Path::new("/Volumes/Some Volume"), &statvfs),
            DeviceType::Unknown
        );
    }
}
