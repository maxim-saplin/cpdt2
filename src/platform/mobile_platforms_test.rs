...
mod tests {
    ...
    mod android_tests {
        ...
        fn test_android_mock_device_enumeration() {
            ...
            assert!(internal_storage.mount_point.to_string_lossy().contains("/storage/emulated"));
            assert_eq!(internal_storage.device_type, DeviceType::SolidState);
            
            let sd_card = &devices[1];
            assert_eq!(sd_card.name, "SD Card");
            assert!(sd_card.mount_point.to_string_lossy().contains("/storage/sdcard"));
            ...
            assert!(app_storage.mount_point.to_string_lossy().contains("/data/data"));
        ...
        fn test_android_mock_app_data_directory() {
            ...
            mock.set_app_data_dir(Some(PathBuf::from("/data/data/com.example.diskspeedtest/files")));
            
            let app_data_dir = mock.get_app_data_dir_instance().unwrap();
            assert!(app_data_dir.to_string_lossy().contains("/data/data"));
            assert!(app_data_dir.to_string_lossy().contains("com.example.diskspeedtest"));
        ...
        fn test_android_mock_storage_access_framework() {
            let mock = MockPlatform::new();
            let saf_path = PathBuf::from("/storage/emulated/0/Android/data/com.example.diskspeedtest");
        ...
        fn test_android_mock_scoped_storage() {
            ...
            mock.add_device(StorageDevice {
                ...
                mount_point: PathBuf::from("/storage/emulated/0/Android/data/com.example.diskspeedtest"),
                total_space: 1024 * 1024 * 1024, // 1GB
                available_space: 512 * 1024 * 1024, // 512MB
                device_type: DeviceType::SolidState,
            });
            ...
            assert!(scoped_device.mount_point.to_string_lossy().contains("Android/data"));
        }
    }
    ...
    mod ios_tests {
        ...
        fn test_ios_mock_device_enumeration() {
            ...
            mock.add_device(StorageDevice {
                ...
                mount_point: PathBuf::from("/var/mobile/Containers/Data/Application/ABC123/Documents"),
            ...
            mock.add_device(StorageDevice {
                ...
                mount_point: PathBuf::from("/var/mobile/Containers/Data/Application/ABC123/Library/Caches"),
            ...
            assert!(documents.mount_point.to_string_lossy().contains("Documents"));
        ...
        fn test_ios_mock_app_data_directory() {
            ...
            let app_data_dir = mock.get_app_data_dir_instance().unwrap();
            assert!(app_data_dir.to_string_lossy().contains("/var/mobile/Containers"));
        ...
        fn test_ios_mock_file_coordination() {
            let mock = MockPlatform::new();
            let coordinated_path = PathBuf::from("/var/mobile/Containers/Data/Application/ABC123/Documents/test.bin");
    ...
    fn test_mobile_platform_space_constraints() {
        ...
        assert!(usage_percentage > 90.0, "Mobile devices often have high storage usage");
    }
    ...
    fn test_mobile_platform_permission_models() {
        ...
        for error_msg in permission_errors {
            mock.simulate_error(PlatformError::InsufficientPermissions(error_msg.to_string()));
