...
pub trait PlatformOps {
    ...
    fn list_storage_devices() -> Result<Vec<StorageDevice>, PlatformError> where Self: Sized;
    
    /// Get the application data directory for the current platform
    fn get_app_data_dir() -> Result<PathBuf, PlatformError> where Self: Sized;
    
    /// Create a file with direct I/O flags for testing
    fn create_direct_io_file(path: &Path, size: u64) -> Result<File, PlatformError> where Self: Sized;
    
    /// Open a file with direct I/O flags
    fn open_direct_io_file(path: &Path, write: bool) -> Result<File, PlatformError> where Self: Sized;
    
    /// Synchronize file system to ensure data is written to disk
    fn sync_file_system(path: &Path) -> Result<(), PlatformError> where Self: Sized;
}
