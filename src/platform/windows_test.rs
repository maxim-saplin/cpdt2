...
mod tests {
    ...
    fn test_windows_device_enumeration() {
        ...
        assert!(
            !devices.is_empty(),
            "Windows should have at least one storage device"
        );
    ...
    fn test_windows_app_data_directory() {
        ...
        assert!(
            app_data_dir.starts_with(&local_app_data),
    ...
    fn test_windows_direct_io_file_creation() {
        ...
        assert_eq!(
            content.len(),
            file_size as usize,
            "File content should match expected size"
        );
    }
    ...
    fn test_windows_drive_type_detection() {
        ...
        let has_typed_device = devices
            .iter()
            .any(|d| matches!(d.device_type, DeviceType::HardDisk | DeviceType::SolidState));
    ...
    fn test_windows_device_name_formatting() {
        ...
        for device in &devices {
            ...
            assert!(
                drive_letter.is_ascii_uppercase(),
    ...
    fn test_windows_mount_point_format() {
        ...
        for device in &devices {
            ...
            assert!(
                mount_str.len() >= 3,
                "Mount point should be at least 3 characters"
            );
            assert!(
                mount_str.ends_with(":\\"),
                "Mount point should end with :\\"
            );
