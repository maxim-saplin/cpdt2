...
use disk_speed_test::test_utils::{
    cleanup::{utils as cleanup_utils, CleanupGuard},
    test_data::{TestDataGenerator, TestDataPattern, TestDataVerifier},
    TestDataManager, TestEnvironment, TestEnvironmentBuilder,
};
#[cfg(feature = "test-utils")]
use std::fs;
#[cfg(feature = "test-utils")]
...
fn test_environment_reset() {
    ...
    let _file1 = env
        .data_manager()
        .create_test_file("test1.dat", 1024)
        .unwrap();
    let _file2 = env
        .data_manager()
        .create_test_file("test2.dat", 2048)
        .unwrap();
...
fn test_sparse_file_creation() {
    ...
    generator
        .create_sparse_file(&sparse_file, 1024 * 1024)
        .unwrap();
...
fn test_pattern_file_creation() {
    ...
    generator
        .create_pattern_file(&pattern_file, 8192, &data_chunks)
        .unwrap();
